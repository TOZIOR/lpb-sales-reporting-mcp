...
function getLastCompleteMonth() {
  ...
  return {
    ...
    label: `mois du ${isoDate(start)} au ${isoDate(end)}`,
  };
}
...
export default async function handler(req: any, res: any) {
  try {
   ...
   const period = getLastCompleteMonth();
    ...
    const subject = `Reporting commercial LPB - ${period.label}`;
    ...
    const resendResponse = await fetch("https://api.resend.com/emails", {
      ...
      body: JSON.stringify({
        ...
        attachments: [
          {
            filename: `reporting_ventes_mensuel_${period.start_date}_${period.end_date}.xlsx`,
            content: excelBase64,
          },
        ],
      }),
